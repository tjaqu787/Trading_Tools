...
def download_data(start_date='1995-01-01', replace=False):
    ...
    if replace:
        data.to_sql('debtandsavings', con, if_exists='replace', index=False, index_label='date')
    else:
        data.to_sql('debtandsavings', con, if_exists='append', index=False, index_label='date')
    con.close()
    return data
